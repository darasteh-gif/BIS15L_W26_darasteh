...
_No, there are columns like region and council with consistent data for the entire study. They are in long format_
```{r}
...
sydneybeaches_long<-sydneybeaches_long %>% 
  ...
  mutate(year=as.integer(year),enterococci_cfu_100ml=as.integer(enterococci_cfu_100ml))
  
```

7. What is the average `enterococci_cfu_100ml` by year for each beach. Think about which data you will use- long or wide.
```{r}
avg_beach_year <- sydneybeaches_long %>% 
  group_by(site, year) %>% 
  summarise(
    avg_enterococci = mean(enterococci_cfu_100ml, na.rm = T),
    .groups = "drop"
  )
