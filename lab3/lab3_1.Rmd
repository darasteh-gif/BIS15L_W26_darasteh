...
```{r}
getwd() #by default you are in the directory you first opened
#setdw() has issues
```
...
tibble equivilent in base r is data.frame()
```{r}
library("tidyverse")
...
```{r}
hbirds <- tibble(Sex,Length,Weight)#each vector is its own column also reffered to as variables
```
...
Let's use lowercase names when we create the data frame. We just changed to lowercase 
here, but we could use any names we wish.
