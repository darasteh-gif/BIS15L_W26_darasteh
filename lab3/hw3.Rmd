...
heightft <- c(29032,28251,28169,27940,27838)
heightme <- c(8849,8611,8586,8516,8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <-  tibble(name,heightft,heightme)
```

**3. What is the mean height of the mountains in feet?**
```{r}
mean(mountains$heightft)
```
...
```{r}
mountains <- tibble(name,heightft,heightme,first_climbed)
```
...
```{r}
mountains$summits <- c(12000,800,280,933,500)
```
...
```{r}
mountains$fatalities <- c(347,96,53,22,48)
...
```{r}
mountains$fatality_rate <- c(mountains$fatalities/mountains$summits)
```
