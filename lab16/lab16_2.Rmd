...
```{}
mv V0G6801W016-Alignment-Descriptions.csv data/blastresults
```
...
```{r}
blastresults <- read_csv("data/blastresults/genome01_blastn.csv")
```

```{r}
glimpse(blastresults)
```

What was the top 5 hits for your blast search ? 
```{r}
blastresults %>% 
  select(`Scientific Name`, `Max Score`) %>% 
  arrange(desc(`Max Score`))
```
...
```{r}
blastresults %>% 
  select(Description, `Scientific Name`, `E value`, Accession) %>% 
...
```{r}
query_sec <- readDNAStringSet("goi_01.txt")
query_sec
```
