...
```{r}
sharks %>% 
  ggplot(aes(x=county,y=incident_num, fill=species))+
  geom_col()+
  coord_flip()+
  labs(title="Incidents per County", x="County", y="Number of Incidents")
```
...
```{r}
sharks %>% 
  ggplot(aes(x=month,y=incident_num,fill=species))+
  geom_col()+
  coord_flip()+
  labs(title="Number of Incidents per Month", x="Month", y="# Incidents")
```
_October_
```{r}
...
sharks %>% 
  arrange(desc(incident_num)) %>% 
  head(10) %>% 
  ggplot(aes(x=time,y=incident_num,fill=species))+
  geom_col()+
  theme(axis.text.x=element_text(angle=90,hjust=0.99))+
  labs(title="Number of Incidents per Time of Day", x="Time of Day", y="# Incidents")
```
...
```{r}
injurys_table %>% 
  pivot_longer(-county, names_to = 'injury', values_to = "n") %>% 
  group_by(county) %>% 
  summarise(total=sum(n)) %>% 
  ggplot(aes(x=county, y=total))+
  geom_col()+
  coord_flip()+
  labs(title = "Total Shark Incidents by County", x="County", y="Number of Incidents")
```
...
sharks %>% 
  ...
  ggplot(aes(x=injury))+
  geom_bar()+
  labs(title="Great White Shark Incident Fatality", x="Fatality", y="Number of Incidents")
```
