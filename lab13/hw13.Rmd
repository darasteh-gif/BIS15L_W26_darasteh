...
```{r}
amniota_nas <- amniota_nas %>% 
  mutate(across(
    c(egg_mass_g, adult_body_mass_g, female_body_mass_g),
    ~ na_if(.x, 0)
  ))
```
...
amniota_nas %>% 
  gg_miss_var()
```
_.SUbspecies and female body mass at maturity have the most NAs._
...
amniota_nas %>% 
  group_by(class) %>% 
  summarize(total_species =n(), egg_mass_na =sum(is.na(egg_mass_g))) %>% 
