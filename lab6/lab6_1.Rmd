...
```{r}
mammals %>%
  mutate(across(c(mass,wean_mass,newborn,litter_size,afr,litters_year),
                ~ifelse(.==-999, NA, .)))
```
...
mammals %>%
  mutate(mass_new=ifelse(mass==-999, NA, mass))%>%
  mutate(mass_kg=mass_new/1000)%>%
...
```{r}
mammals %>%
  select(genus,species,newborn,gestation)%>%
  mutate(newborn_new=ifelse(newborn==-999.00,NA, newborn))%>%
  mutate(gestation_new=ifelse(gestation==-999.00,NA, gestation))%>%
  ggplot(mapping=aes(x=gestation_new,y=log10(newborn_new)))+
         geom_point()
```
