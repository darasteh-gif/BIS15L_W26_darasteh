...
getwd()
setwd("data")
gabon <- read.csv("IvindoData_DryadVersion.csv")%>%clean_names() 
```
...
ggplot( #specifies the data
  mapping=aes(x=land_use))+ #map the aesthetics
  geom_bar(mapping=aes(color=land_use))+
  labs(title= "Transects per Land Use Type")
...
gabon%>%
ggplot( #specifies the data
  mapping=aes(x=land_use,y=veg_rich))+ #map the aesthetics
  geom_boxplot(mapping=aes(color=land_use))+
  labs(title= "Vegetation Richness per Land Use Type")
...
gabon%>%
ggplot( #specifies the data
  mapping=aes(x=land_use,y=rich_all_species))+ #map the aesthetics
  geom_boxplot(mapping=aes(color=land_use))+
  labs(title= "Richness per Land Use Type")
...
gabon%>%
ggplot( #specifies the data
  mapping=aes(x=veg_rich,y=rich_mammal_species))+ #map the aesthetics
  geom_point(mapping=aes(color=land_use))+
  labs(title= "Vegetation Richness vs. Mammal Richness")+
...
```{r}
gabon%>%
  filter(veg_rich>15 | (veg_dbh<60 & veg_dbh>50))
```
