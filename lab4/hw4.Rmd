...
ggplot(data=msleep, #specifies the data
      mapping=aes(x=vore))+ #map the aesthetics
  geom_bar(mapping=aes(color=vore))+
  labs(title= "Mammals per Vore")

```
##Herbivores are Most Represented
...
**geom_bar()**5. Use `summary()` to get a quick idea of the range of values for the variable `bodywt`.**
```{r}
...
ggplot(data=msleep,
  ...
  labs(title= "Sleep Total by Body Weight", x= "Body Weight", y= "Sleep Total")
...
ggplot(data=msleep,
      ...
      geom_boxplot(mapping=aes(color=vore))+
  scale_y_log10()+
  labs(title= "Brain Weight by Vore Type")
```
