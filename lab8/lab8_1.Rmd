...
```{r}
elephants %>% 
  summarize(across(where(is.numeric,mean=~mean())))
```
...
elephants %>% 
  ...
  summarise(mean_age=mean(estimated_age_years, na.rm=T),
            ...
            sd_age=mean(estimated_age_years, na.rm=T),
            n=n(),
            .groups='keep')
```
...
msleep %>% 
  group_by(vore) %>% 
  summarize(mean=mean(bodywt,na.rm=T),
            min=min(bodywt,na.rm=T),
