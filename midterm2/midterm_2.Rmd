...
polarbear_nas %>% 
  ...
  geom_bar()+
  labs(title="Numer of Bears sampled by Sex", x="Sex", y="Number Sampled")
...
```{r}
polarbear_nas %>% 
  mutate(numbersampled=n_distinct(bear_id)) %>% 
  ggplot(aes(x=year, y=numbersampled))+ 
  geom_line()+
  facet_wrap(~sex)+
...
#polarbear_nas %>% 
 # filter(fasting=="yes") %>% 
  #ggplot(x=count(bear_id), color=popn, fill=popn)+
...
```
```{r}
#latitude <- c(62.46, 72.7)
#longitude <- c(0172.2, -134.8)
#boundbox <- make_bbox(longitude, latitude, f=.03)
...
```{r}
#ggmap(map1)+
 # geom_point(data=polarbear_nas, aes(longitude, latitude), size=1, #color="popn", alpha=.8)+
...
ui <- fluidPage(
    ...
    selectInput("x",
              "Select Population",
              choices=sort(unique(polarbear_nas$popn)),
              selected ="SB"),
    selectInput("y",
              "Select Variable",
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    polarbear_nas %>% 
     filter(input$x==.data[["popn"]]) %>% 
      ggplot(aes(x=.data(input$y)))+
      geom_boxplot()
