...
ggplot(data=homerange, 
       mapping=aes(x=class))+
         geom_bar(mapping=aes(color=class))
```
...
ggplot(data=homerange, 
       mapping=aes(x=trophic.guild))+
         geom_bar(mapping=aes(color=trophic.guild))
```
...
homerange%>% 
  ...
  filter(trophic.guild=="herbivore",class=="mammalia", mean.hra.m2>1000000)%>%
...
homerange%>% 
  ...
  filter(trophic.guild=="carnivore",class=="mammalia", mean.hra.m2>1000000)%>%
  select(common.name, order, mean.hra.m2, mean.mass.g)%>%
  arrange(desc(mean.hra.m2))%>%
  slice_head(n=10)
...
ggplot(data=homerange, 
       mapping=aes(x=log10.hra, y=log10.mass))+
         geom_point()+
  labs(title="Mass vs. Homerange Size", x="Log10 Homerange Size", y="Log10 Mass(g)")
```
...
```{r}
homerange%>%
  classdata <- c(filter(class=="mammalia"| class=="aves"))%>%
