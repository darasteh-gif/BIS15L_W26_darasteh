...
gap %>% 
  ...
  labs(title="US GDP 2000-2025", x="Year", y="GDP ")
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_histogram(color="black",fill="darkcyan",bins=40)+
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_density(color="black",fill="darkcyan",alpha=0.7)+
  labs(title="Distribution of Life Expectancy")
```
...
gap %>% 
  ggplot(aes(x=lex))+
  geom_histogram(aes(y=after_stat(density)), color="black",fill='darkcyan')+
  geom_density(color="white", fill='white', alpha=.3)
```
...
gap %>% 
  ...
  ggplot(aes(x=lex, color=year, fill = year))+#color and fill by year
  geom_density(alpha=.3)+
  labs(title="Distribution of Life Expectancy, 1930 and 1950", x="Life Expectancy", y=NULL, color='Year', fill='Year')
```
...
income_cat2000<-gap %>% 
  filter(year==2000) %>% 
  mutate(income_catagory=case_when(gdp<=1100 ~ "low",
                                   gdp>=1100 & gdp<=4500 ~"low_middle", 
                                   gdp>4500 & gdp<=13900 ~"upper_middle",
                                   gdp>13900~"high")) %>% 
  select(country, gdp, income_catagory) %>% 
  mutate(income_category=factor(income_catagory,
